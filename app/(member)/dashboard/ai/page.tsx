  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        <Card className="flex min-h-[560px] flex-col rounded-lg p-0">
          ...
          <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
            ...
            {params?.q ? (
              <div className="ml-auto flex max-w-[78%] flex-row-reverse gap-3">
                ...
                <div className="rounded-lg bg-surface-2 px-4 py-3 text-sm leading-7 text-text-1">{params.q}</div>
              </div>
            ) : null}
          ...
          <div className="border-t border-border px-5 py-4">
            ...
            <form action={submitAiQueryAction} className="flex gap-2">
              <textarea
                ...
                className="min-h-11 flex-1 resize-none rounded-lg border border-border px-3 py-2.5 text-sm"
                placeholder="例如：誰的專業是 AI落地"
              />
              <button type="submit" className="rounded-lg bg-text-1 px-5 py-2.5 text-sm font-semibold text-white">送出查詢</button>
            </form>
          </div>
        </Card>
          ...
          <Card className="rounded-lg p-5">
            ...
            <div className="mt-4 space-y-3">
              {data.reminderPreview.slice(0, 4).map((item) => (
                <div key={item.memberName} className="rounded-lg border border-border p-3 text-sm">
                  <p className="font-medium text-text-1">{item.memberName}</p>
                  <p className="mt-1 line-clamp-2 text-text-2">{item.message}</p>
                </div>
              ))}
              {data.reminderPreview.length === 0 ? <div className="rounded-lg border border-dashed border-border p-4 text-sm text-text-2">本週已無未提交會員。</div> : null}
            </div>
          </Card>
