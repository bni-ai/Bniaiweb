  ...
  try {
    ...
    return (
      <div className="space-y-5">
        <div>
          <p className="text-sm text-text-2">一對一視訊</p>
          <h1 className="text-3xl font-black">一對一視訊入口</h1>
          <p className="mt-2 text-sm text-text-2">只有已確認且在時間窗口內的預約可以進入同一個線上會議室。</p>
        </div>
        <Card className="rounded-2xl p-5">
          <div className="grid gap-5 md:grid-cols-[1fr_220px] md:items-center">
            ...
            <a href={session.meetUrl} target="_blank" className="inline-flex justify-center rounded-2xl bg-primary px-5 py-3 text-sm font-semibold !text-white">
              進入視訊會議
            </a>
          </div>
          <div className="mt-5 rounded-2xl border border-dashed border-border p-4 text-sm text-text-2">會議入口只在預約前 30 分鐘到預約後 90 分鐘內開放。</div>
        </Card>
      </div>
    );
  } catch (error) {
