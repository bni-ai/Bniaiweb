...
export default async function SubmissionPage({ searchParams }: { searchParams?: Promise<{ week?: string; reminded?: string }> }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 rounded-[28px] border border-[#ead9cc] bg-white/80 p-5 shadow-[0_16px_44px_rgba(17,24,39,0.04)] md:flex-row md:items-end md:justify-between">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <form action={sendBulkReminderAction}>
            <input type="hidden" name="week_date" value={weekDate} />
            <Button type="submit" variant="secondary" className="rounded-full px-5">提醒未提交會員</Button>
          </form>
        </div>
      </div>
      ...
      <Card className="overflow-hidden rounded-[28px]">
        <table className="w-full text-sm"><thead className="bg-[#2a2522] text-left text-white"><tr><th className="p-4">會員</th><th className="p-4">狀態</th><th className="p-4">本週可提供</th><th className="p-4">本週需求</th><th className="p-4">操作</th></tr></thead><tbody>
          {data.rows.map(({ member, brief, remindedAt }: any) => (
            <tr className="border-t border-border align-top" key={member.id}>
              <td className="p-4"><b>{member.chinese_name}</b><p className="text-xs text-text-2">{member.specialty_title || member.email}</p></td>
              <td className="p-4"><span className="inline-flex rounded-full bg-surface-2 px-3 py-1 text-xs font-semibold text-text-1">{brief?.status === "submitted" ? "已提交" : brief ? "草稿" : "未提交"}{brief?.submitted_late ? " / 逾期" : ""}{brief?.approved_at ? " / 已核准" : ""}{remindedAt ? " / 已提醒" : ""}</span></td>
              <td className="max-w-xs p-4 text-text-2">{brief?.have_this_week || "-"}</td><td className="max-w-xs p-4 text-text-2">{brief?.want_this_week || "-"}</td>
              <td className="space-y-2 p-4">
                <form action={saveBriefAction} className="grid gap-2">
                  ...
                  <textarea name="have_this_week" className="rounded-2xl border border-border bg-surface-2 p-2" defaultValue={brief?.have_this_week || ""} placeholder="本週可提供" />
                  <textarea name="want_this_week" className="rounded-2xl border border-border bg-surface-2 p-2" defaultValue={brief?.want_this_week || ""} placeholder="本週需求" />
                  <select name="status" className="rounded-2xl border border-border bg-surface-2 p-2" defaultValue={brief?.status || "draft"}><option value="draft">草稿</option><option value="submitted">已提交</option></select>
                  <Button type="submit" className="w-full rounded-full">儲存</Button>
                </form>
                {brief?.id ? <form action={approveBriefAction}><input type="hidden" name="brief_id" value={brief.id} /><Button type="submit" variant="secondary" className="w-full rounded-full">核准</Button></form> : null}
              </td>
            </tr>
          ))}
        </tbody></table>
