...
- [ ] 2.1 建立 `members` table migration：包含 id (uuid), email (unique), member_number (integer, unique), chinese_name, english_name, line_name, specialty_title, specialty_description, general_referral, ideal_referral, dream_referral, photo_url, role (enum: member/officer/president), auth_uid (uuid, nullable), created_at, updated_at。啟用 RLS：member 只能讀寫自己（auth.uid() = auth_uid），officer/president 可讀寫全部。驗證：`supabase db push` 成功，Supabase dashboard 顯示 table + RLS policies。[Tool: Sonnet]
...
- [ ] 2.9 建立 `ai_settings` table migration：id, provider (enum: claude/gemini/openai), api_key_encrypted (text), model_name, is_active (boolean), created_at, updated_at。RLS：只有 president 可讀寫。驗證：migration 成功。[Tool: Sonnet]
...
- [ ] 3.2 建立 `/auth/callback` route handler，處理 OAuth 回調：查 `members` table 比對 email → 綁定 auth_uid → 根據 role 導向 `/dashboard` 或 `/admin`。非會員 email 導向 `/not-member` 錯誤頁。驗證：已存在 email 登入 → 到 dashboard；不存在 email → 到 not-member 頁。[Tool: Sonnet]
- [ ] 3.3 建立 middleware.ts 保護路由：未登入 → redirect /login；member 存取 /admin/* → redirect /dashboard；officer/president 可存取 /admin/*。驗證：未登入訪問 /dashboard → 跳轉 /login。[Tool: Sonnet]
...
- [ ] 7.5 建立 `/admin/settings` 系統設定頁面：submission deadline、reminder time、chapter info、AI provider 設定。驗證：改 deadline → 新 deadline 在填寫頁面顯示正確。[Tool: Sonnet]
...
- [ ] 15.2 端到端驗收：(1) 匯入 CSV → 36 會員建立；(2) member 登入 → 填寫 weekly brief → submit；(3) officer 登入 → dashboard 顯示填寫進度 → 產出簡報 → 發布；(4) 公開 URL 可瀏覽完整簡報。驗證：以上 4 步全部成功完成。[Tool: Sonnet]
