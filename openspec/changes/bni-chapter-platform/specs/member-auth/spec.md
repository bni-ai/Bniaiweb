...
The system SHALL enforce two roles: `admin` (幹部/主席) and `member` (一般會員). The role SHALL be stored in the `members.role` column. Members with a non-'無' `position` value (主席/副主席/秘書財務/資訊組長/成長協調/活動協調/導師協調/教育協調/來賓接待) SHALL be assigned `role='admin'`. All other members SHALL be assigned `role='member'`. Supabase RLS policies SHALL restrict data access based on role.
...
- **WHEN** a user with role `admin` accesses any `/admin/*` route
- **THEN** the system SHALL render the admin page with full management capabilities

#### Scenario: Admin logs in

- **WHEN** a user with role `admin` completes Google OAuth
- **THEN** the system SHALL redirect to `/admin` after binding auth_uid
