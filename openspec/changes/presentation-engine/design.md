...
Backward-compat: `/presentation/[id]` SHOULD remain supported as a lookup alias that resolves `id -> week_date` and redirects to canonical route.
...
**Interface:**
- `app/presentation/[week-date]/page.tsx` loads the presentation, resolves all required linked data, validates slide_order, and renders the slide list
- `app/presentation/[id]/page.tsx` (compat alias) resolves record by `id` then redirects to `/presentation/[week-date]`
